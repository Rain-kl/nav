...
router.get("/admin", (request, env, ctx) =>
  serveStatic(request, env, ctx, "/admin.html"),
);
router.get("/admin/", (request, env, ctx) =>
  serveStatic(request, env, ctx, "/admin.html"),
);
...
async function handleLogin(request, env) {
  ...
  await env.SESSIONS.put(token, "active", {
...
async function handleGetAdminData(request, env) {
  const fullData = await readFullData(env);
  return jsonResponse({ success: true, data: sanitiseData(fullData) });
}
...
async function handlePasswordUpdate(request, env) {
  ...
  if (!cleanNewPassword || cleanNewPassword.length < 6) {
    return jsonResponse(
      { success: false, message: "æ°å¯ç é¿åº¦è³å°ä¸º 6 ä½ã" },
      400,
    );
  }
  ...
  if (!admin || !admin.passwordHash || !admin.passwordSalt) {
    return jsonResponse(
      { success: false, message: "å¯ç ä¿®æ¹åè½æä¸å¯ç¨ã" },
      500,
    );
  }
  ...
  if (isSameAsOld) {
    return jsonResponse(
      { success: false, message: "æ°å¯ç ä¸è½ä¸å½åå¯ç ç¸åã" },
      400,
    );
  }
  ...
  return jsonResponse({
    success: true,
    message: "å¯ç å·²æ´æ°ï¼ä¸æ¬¡ç»å½è¯·ä½¿ç¨æ°å¯ç ã",
  });
}
...
function handleFetchLogo(request, env) {
  try {
    ...
    if (!targetUrl || typeof targetUrl !== "string" || !targetUrl.trim()) {
      return jsonResponse(
        { success: false, message: "ç¼ºå°ææç targetUrl åæ°" },
        400,
      );
    }
    ...
    if (!domain) {
      return jsonResponse(
        { success: false, message: "æ æ³ä»é¾æ¥ä¸­æåååã" },
        400,
      );
    }
...
async function requireAuth(request, env) {
  ...
  if (!raw || !raw.startsWith(AUTH_HEADER_PREFIX)) {
    return jsonResponse(
      { success: false, message: "è¯·ç»å½ååæ§è¡æ­¤æä½ã" },
      401,
    );
  }
  ...
  if (!session) {
    return jsonResponse(
      { success: false, message: "ç»å½ç¶æå·²å¤±æï¼è¯·éæ°ç»å½ã" },
      401,
    );
  }
...
function normaliseCollection(value, { label, type }) {
  if (!Array.isArray(value))
    throw new Error(`${label} æ°æ®æ ¼å¼ä¸æ­£ç¡®ï¼åºä¸ºæ°ç»ã`);
  const seen = new Set();
...
function normaliseItem(input, type) {
  if (!input || typeof input !== "object")
    throw new Error("æ°æ®é¡¹æ ¼å¼ä¸æ­£ç¡®ã");
  ...
  const payload = {
    ...
    icon: normaliseIconUrl(input?.icon),
  };
  if (type === "bookmarks") {
    payload.category =
...
function resolveApiKeyFromWeather(weather, env) {
  ...
  const fromEnv =
    typeof env?.QWEATHER_API_KEY === "string"
      ? env.QWEATHER_API_KEY.trim()
      : "";
  return fromEnv;
}
...
function resolveQWeatherApiHost(weather, env) {
  ...
  const fromEnv = normaliseApiHost(
    typeof env?.QWEATHER_API_HOST === "string" ? env.QWEATHER_API_HOST : "",
  );
    ...
    apps: [
      {
        ...
        description: "åä½å¼çé¢è®¾è®¡å·¥å·ã",
        icon: "ð¨",
      },
      {
        id: "app-notion",
        ...
        description: "å¤åä¸çç¬è®°ä¸ç¥è¯ç®¡çå¹³å°ã",
        icon: "ðï¸",
      },
      {
        id: "app-slack",
        name: "Slack",
        url: "https://slack.com/",
        description: "å¢éå³æ¶æ²éä¸åä½ä¸­å¿ã",
        icon: "ð¬",
      },
      {
        id: "app-github",
        ...
        description: "ä»£ç æç®¡ä¸åä½å¹³å°ã",
        icon: "ð",
      },
      {
        id: "app-canva",
        ...
        description: "ç®åæç¨çå¨çº¿è®¾è®¡å·¥å·ã",
        icon: "ðï¸",
      },
    ],
    bookmarks: [
      {
        ...
        description: "èç¦å¼æºä¿¡æ¯ä¸ææ¯ç¤¾åºã",
        icon: "ð",
        category: "ææ¯ç¤¾åº",
      },
      {
        ...
        name: "å°æ°æ´¾",
        url: "https://sspai.com/",
        description: "å³æ³¨æçå·¥å·ä¸çæ´»æ¹å¼çåªä½ã",
        icon: "ð°",
        category: "æçä¸çæ´»",
      },
      {
        ...
        description: "é®ç­ä¸ç¥è¯åäº«ç¤¾åºã",
        icon: "â",
        category: "ç¥è¯å­¦ä¹ ",
      },
      {
        ...
        description: "å´è¶£ç¤¾äº¤ä¸èµè®¯èåå¹³å°ã",
        icon: "ð®",
        category: "èµè®¯èå",
      },
      {
        ...
        name: "ç¨åæé",
        url: "https://juejin.cn/",
        description: "å¼åèææ¯ç¤¾åºä¸ä¼è´¨åå®¹ã",
        icon: "ð¡",
        category: "ææ¯ç¤¾åº",
      },
    ],
...
async function fetchAndCache(
  url,
  ctx,
  successTtlSeconds = WEATHER_CACHE_TTL_SECONDS,
  errorTtlSeconds = ERROR_CACHE_TTL_SECONDS,
) {
...
async function geocodeCity(
  ...
  throw (
    lastError || createWeatherError("Failed to resolve city location.", 502)
  );
}
...
async function fetchQWeatherNowByLocation(
  location,
  apiKey,
  env,
  ctx,
